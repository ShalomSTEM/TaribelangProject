...
class Player(RoomObject):
    def __init__(self, room, x, y, size):
        ...
        self.size = size
        self.set_image(os.path.join("Images/MilbiL1", "sprite_3.png"), size, size)
        self.depth = 5
    ...
    def key_pressed(self, key):
        ...
        if key[pygame.K_RIGHT]:
            self.set_image(
                os.path.join("Images/MilbiL1", "sprite_3.png"), self.size, self.size
            )
            Globals.player_x += 1
        if key[pygame.K_LEFT]:
            self.set_image(
                os.path.join("Images/MilbiL1", "sprite_1.png"), self.size, self.size
            )
            Globals.player_x -= 1
